    ...
    <main className="pt-32 pb-24 bg-white min-h-screen px-6">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-16">
          <div>
            <h1 className="text-4xl font-light text-[#111111] tracking-tight mb-4">Mis Direcciones</h1>
            <p className="text-sm text-[#2B2B2B]/50 font-light">Gestiona tus direcciones de envío</p>
          </div>
        ...
        {isLoading ? (
          <div className="text-center py-20">
            <p className="text-[#2B2B2B]/40 text-sm font-light uppercase tracking-widest">Cargando direcciones...</p>
          </div>
        ) : addresses.length === 0 ? (
          <div className="text-center py-20 space-y-6">
            <MapPin size={48} className="mx-auto text-[#2B2B2B]/20" />
            <p className="text-[#2B2B2B]/40 text-sm font-light uppercase tracking-widest">No tienes direcciones guardadas</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {addresses.map(addr => (
              <div key={addr.id} className={`bg-[#EDEDED] p-8 relative group transition-all ${addr.isDefault ? 'ring-2 ring-[#3A4A3F]' : ''}`}>
                {addr.isDefault && (
                  <span className="absolute top-4 right-4 text-[10px] uppercase tracking-widest font-bold text-[#3A4A3F] flex items-center gap-1">
                    <Star size={12} fill="currentColor" /> Predeterminada
                  </span>
                )}
                <h3 className="text-sm font-bold uppercase tracking-widest mb-4">{addr.label}</h3>
                <div className="text-sm text-[#2B2B2B]/60 font-light space-y-1">
                  <p>{addr.street}</p>
                  <p>{addr.city}, {addr.state} {addr.zip}</p>
                  <p>{addr.country}</p>
                </div>
                <div className="flex gap-4 mt-6 pt-4 border-t border-[#2B2B2B]/10">
                  <button onClick={() => openEdit(addr)} className="text-[10px] uppercase tracking-widest font-bold text-[#2B2B2B]/40 hover:text-[#111111] flex items-center gap-1">
                    <Pencil size={12} /> Editar
                  </button>
                  {!addr.isDefault && (
                    <button onClick={() => handleSetDefault(addr.id)} className="text-[10px] uppercase tracking-widest font-bold text-[#2B2B2B]/40 hover:text-[#3A4A3F] flex items-center gap-1">
                      <Star size={12} /> Predeterminar
                    </button>
                  )}
                  <button onClick={() => setDeleteId(addr.id)} className="text-[10px] uppercase tracking-widest font-bold text-[#2B2B2B]/40 hover:text-red-500 flex items-center gap-1">
