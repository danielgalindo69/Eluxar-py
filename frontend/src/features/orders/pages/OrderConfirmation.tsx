      ...
      <div className="max-w-2xl mx-auto text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
            ...
            <div className="space-y-6">
              {orderItems.map((item, i) => (
                <div key={i} className="flex items-center justify-between border-b border-[#2B2B2B]/10 pb-4 last:border-0">
                  ...
                  <span className="text-sm font-bold">{formatPrice(item.price * item.quantity)} COP</span>
                </div>
              ))}
            </div>
          ) : (
          ...
          <div className="pt-4 border-t border-[#2B2B2B]/10 flex justify-between items-end">
            ...
            <span className="text-2xl font-light tracking-tight">{orderTotal > 0 ? formatPrice(orderTotal) : '0'} COP</span>
          </div>
        </motion.div>
